...
9. ✅ **Dashboard visual redesign, phase 1 (light/dark theme + per-business brand color)** -- DONE 2026-07-01. See `docs/activity-log.md` for full detail. Built the CSS-variable theming infrastructure (light/dark tokens, brand color derivation) and fully applied it to `Layout.jsx`, `DashboardPage.jsx`, and `AppointmentsPage.jsx`. **Next: roll the same brand-color treatment out to the remaining ~17 dashboard pages** (Customers, Services, Technicians, Contacts, SMS, Notifications, OnCall, Settings, Billing, Businesses, Onboarding, Setup, Login, etc.) -- each needs manual review to convert "chrome" blue (buttons/links/focus rings) to `brand` while leaving genuine status/semantic blue (e.g. "in_progress", "New" contact badge, "active" SMS badge, plan badges, category tags) untouched. Not a mechanical find-replace -- blue is overloaded as both brand-chrome and status meaning throughout the app, confirmed by grep audit 2026-07-01.
